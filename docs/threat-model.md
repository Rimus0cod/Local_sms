...
### Поддельный инвайт или повторное использование старой ссылки

Риск:

- злоумышленник подсовывает изменённый invite-link
- старая ссылка используется повторно после исчерпания лимита

Защита:

- invite-link подписан через HMAC-SHA256
- сервер хранит `invite_id`, лимит использований и срок действия
- join flow проверяет и подпись, и серверное состояние usage/expiry перед регистрацией устройства
